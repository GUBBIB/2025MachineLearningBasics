...
```
- **필터(커널)** 의 크기가 ``3x3``이고 채널이 ``32개`` 라고 했을 때, 각 **필터(커널)** 는 ``9개`` 의 값(가중치)을 가지고 있고 ``채널 x 필터크기`` 개 만큼 값을 가지므로 ``3 x 3 x 32 = 288개``가 있다.
- 하지만 ``Conv2D`` 의 매개변수 ``use_bias`` 의 기본값이 ``True`` 이기 때문에 ``편향``을 사용하게 되므로 **편향의 개수** ``1`` 을 더해줘야 한다.
- 마지막으로, ``출력 채널 개수`` 를 **곱해주면** 해당 층에서 **조정되는 가중치와 편향의 총 개수(Param 수)** 가 계산된다.
